...
int main (void)
{
    ...
    int frag_target_ID=1, fragID, fragPLlength, Nbfrag, packetLength=0, tlen, s, rc, coneID;
    unsigned char packet[2500], temp[2500];
    //unsigned char dstIP[4], OrgnldstIP[4], srcMAC[6], dstMAC[6], packet[2048], temp[2048];
    struct sockaddr myaddr, from;
    socklen_t fromlen=0;

    memset(packet, 0x00, 2500);

    const unsigned char dstIP[4] = {0xc0, 0xa8, 0x02, 0x64};
    const unsigned char OrgnldstIP[4] = {0xc0, 0xa8, 0x00, 0x64};
    const unsigned char dstMAC[6] = {0x00, 0x23, 0x18, 0xa5, 0x32, 0x97};
    const unsigned char srcMAC[6] = {0xc8, 0xa0, 0x30, 0xa8, 0x53, 0xc2};
    /*
...
//printf("310 0X%X 0X%X 0X%X 0X%X 0X%X 0X%X\n", dstMAC[0], dstMAC[1], dstMAC[2], dstMAC[3], dstMAC[4], dstMAC[5]);

    //BB2 eth0 check later
    memset(srcMAC, 0x00, 6);
    srcMAC[0] = 0xc8;
    srcMAC[1] = 0xa0;
    srcMAC[2] = 0x30;
    srcMAC[3] = 0xa8;
   ...
   memset((char *)sharedMem_int, 0x00, 10000);
    ...
    int packetNumber=0;
		...
		do
		{
			...
			//printf("0 received!\n");
			//printf("423 : 0X%X 0X%X 0X%X 0X%X 0X%X 0X%X\n", dstMAC[0], dstMAC[1], dstMAC[2], dstMAC[3], dstMAC[4], dstMAC[5]);
			...
			coneID = *((int *)packet_byte);
			//printf("coneID = %d, fragID = %d, targetID=%d, Nbfrag=%d\n", coneID, fragID, frag_target_ID, Nbfrag);
			    ...
			    //if(fragID == 4)
				    //printf("!!!!!!!!!!!!!!!!!! fragID =4, length = %d     !!!!!!!!!!!!!!!!!!!!!!!!!!!!\n", fragPLlength);
                            if(fragID == Nbfrag)
                            {
                                if(memcmp(packet+30, OrgnldstIP, 4) == 0) //compare IP and send out the packet
                                { //printf("444 : IP address found");
				    //printf("dstMAC : 0X%X 0X%X 0X%X 0X%X 0X%X 0X%X\n", dstMAC[0], dstMAC[1], dstMAC[2], dstMAC[3], dstMAC[4], dstMAC[5]);	
                                    memcpy(packet, dstMAC, 6);
				    //printf("packet : 0X%X 0X%X 0X%X 0X%X 0X%X 0X%X\n", packet[0], packet[1], packet[2], packet[3], packet[4], packet[5]);
                                    
				    //printf("srcMAC : 0X%X 0X%X 0X%X 0X%X 0X%X 0X%X\n", srcMAC[0], srcMAC[1], srcMAC[2], srcMAC[3], srcMAC[4], srcMAC[5]);
				    memcpy(packet+6, srcMAC, 6);

				    //printf("dstIP : 0X%X 0X%X 0X%X 0X%X\n", dstIP[0], dstIP[1], dstIP[2], dstIP[3]);
				   ...
				   /* for(i=0;i<packetLength;i++)
				    ...
				    //printf("#######0X%X 0X%X 0X%X 0X%X 0X%X 0X%X#####\n", packet[0], packet[1], packet[2], packet[3], packet[4], packet[5]);
                        ...
                        {
			    printf("!!!!!!!!!!!!cone_ID = %d,  frag_ID = %d, frag_target_ID = %d !!!!!!!!!!!!\n",coneID, fragID, frag_target_ID);
		            fragPLlength = *((int *)packet_byte+1);
			    int i;
                            for (i=0; i<fragPLlength ;i++)
				    printf("%X ", packet_byte+FRAG_HEADER_LENGTH+i);
			    printf("\n");
			...
			//printf("1 received!\n");
			//printf("516 : 0X%X 0X%X 0X%X 0X%X 0X%X 0X%X\n", dstMAC[0], dstMAC[1], dstMAC[2], dstMAC[3], dstMAC[4], dstMAC[5]);
				/*
			...
			//printf("coneID = %d, fragID = %d, targetID = %d, Nbfrag=%d\n",coneID, fragID, frag_target_ID, Nbfrag);
			    ...
			    //if(fragID == 4)
				    ...
				    //printf("$$$$$0X%X 0X%X 0X%X 0X%X 0X%X 0X%X$$$\n", packet[0], packet[1], packet[2], packet[3], packet[4], packet[5]);
                        ...
                        {
                            printf("!!!!!!!!!!!!!!!!!!!!!!!!! coneID = %d, frag_ID = %d, target_frag_ID = %d !!!!!!!!!!!!!!!!\n", coneID, fragID, frag_target_ID);
                            fragPLlength = *((int *)packet_byte+1);
			    int i;
			    for (i=0 ; i<fragPLlength;i++)
				   printf(" %X", packet_byte+FRAG_HEADER_LENGTH+i);
