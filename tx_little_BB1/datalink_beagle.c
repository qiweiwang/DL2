...
int main()
{
   ...
   // ********************************************************************
    ...
    while(1)
    {
        ...
        //printf("this packet has length %d\n", length);
       ...
       // printf("\n");
        ...
        //if (r_length != r)
        if (r>r_length+4)
             printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! r = %d, r_length = %d !!!!!!!!!!!!!!!!!!!!\n", r, r_length);
        else if (r == r_length+4)
            ;
        else
        {   
            printf("!!!!!!!!!!!!!!!!!!!!!!! SAME LENGTH r = %d !!!!!!!!!!!\n", r);
            //continue;
        }
        CondID++;
        fragID = 0;
        Nbfrag = (int)ceil((double)r_length/FRAG_PAYLOAD_LENGTH);
        //int origin_r_length = r_length;
        while(r_length > 0)
        {   
            struct timespec tim;
            tim.tv_sec = 0;
	    ...
	    /*if(fragID == 1)
            {
		int c = 16;
		printf("\n*******first 6 bytes of first fragment:********\n");
                if(fragment[c] == 0x12)
		{
                	for (; c < 22; c++)
                	{
